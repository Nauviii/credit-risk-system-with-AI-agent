...
def _frame() -> pl.DataFrame:
    ...
    return pl.DataFrame({
        ...
        "term": [" 36 months", " 36 months", " 60 months", " 36 months", " 36 months", " 36 months"],
...
def test_first_reliable_year_picks_earliest_year_below_threshold():
    ...
    lookup = dict(zip(result["feature"], result["first_year"]))
...
def test_hazard_cumulative_share_reaches_one_per_term():
    ...
    assert result.group_by("term_months").agg(pl.col("cum_share").max())["cum_share"].to_list() == [1.0, 1.0]
...
def test_proxy_check_detects_recovery_contamination():
    ...
    gap = dict(zip(result["has_recovery"], result["med_gap"]))
