    ...
    return pl.DataFrame(
        {
            ...
            "grade": grade,
            "int_rate": np.where(
                grade == "A", 8.0, np.where(grade == "B", 12.0, np.where(grade == "C", 18.0, 24.0))
            ),
            "term_months": rng.choice([36, 60], n),
            "fico_range_low": rng.uniform(660, 750, n),
            ...
            "inq_last_6mths": rng.integers(0, 5, n),
            "purpose": rng.choice(["debt_consolidation", "credit_card"], n),
            "acc_open_past_24mths": rng.integers(0, 10, n),
            "bc_open_to_buy": rng.uniform(0, 20000, n),
            "avg_cur_bal": rng.uniform(0, 50000, n),
            "tot_hi_cred_lim": rng.uniform(0, 200000, n),
            "tot_cur_bal": rng.uniform(0, 100000, n),
            "num_tl_op_past_12m": rng.integers(0, 5, n),
            "total_bc_limit": rng.uniform(0, 30000, n),
            "mort_acc": rng.integers(0, 3, n),
            "installment": rng.uniform(50, 1200, n),
            "percent_bc_gt_75": rng.uniform(0, 100, n),
            "num_actv_rev_tl": rng.integers(0, 10, n),
            "bc_util": rng.uniform(0, 100, n),
            "num_rev_tl_bal_gt_0": rng.integers(0, 10, n),
            "mo_sin_rcnt_tl": rng.integers(0, 60, n),
            "total_rev_hi_lim": rng.uniform(0, 100000, n),
            "mo_sin_rcnt_rev_tl_op": rng.integers(0, 60, n),
            "mths_since_recent_bc": rng.integers(0, 60, n),
            "mo_sin_old_rev_tl_op": rng.integers(0, 200, n),
